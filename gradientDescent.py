...
def GetValue(f,args,point):
    if isinstance(f,np.ndarray):
        return np.array([i.evalf(subs=GetSubs(args,point)) for i in f]).astype(decimal.Decimal)
    else:
        return np.array([f.evalf(subs=GetSubs(args, point))]).astype(decimal.Decimal)
...
if __name__ == "__main__":
    ...
    for j in range(4):
        ...
        for i in range(4):
            ...
            # Draw picture
            color=['r-','g-','b-','y-']
            strb=['0.1','0.2','0.3','0.4']
            #plt.plot(range(len(eStack)),[math.log(x) for x in eStack],color[i],label="β="+strb[i])
            plt.semilogy(range(len(eStack)),eStack,color[i],label="b="+strb[i])
            plt.legend()
            # Set axis
            plt.xlabel("Iteration Times")
            plt.ylabel("f(x^k)-p*")
            #plt.set_yticklabels('0.60','0.70')
    plt.show()
